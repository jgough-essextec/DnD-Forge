...
Full campaign lifecycle management — create, view, edit, archive, and delete campaigns with house rules, join codes, and character assignment. Extend the Django REST API and PostgreSQL schema to support campaigns as first-class entities.

## Stories

| Story | Title | Tasks | Focus |
|-------|-------|-------|-------|
| 33.1 | Campaign Data Model & Store | 5 | TypeScript interfaces, Django models / API endpoints, React Query hooks, Zustand UI store |
...
- **Phase 1-3:** Character data model, Django REST API / PostgreSQL setup, export/import system
...
- `stores/useCampaignStore.ts` (Zustand — UI state: selected campaign, filters)
- `hooks/useCampaign.ts` (React Query — server state: campaign CRUD via API)
- `types/campaign.ts`
...
- **Error Handling** gaps in duplicate campaign names, joinCode collisions, auto-save failures, and API error scenarios
- **Loading/Empty States** missing loading spinners during API operations across all stories
