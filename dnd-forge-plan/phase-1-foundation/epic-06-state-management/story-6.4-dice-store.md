...
## Description
As a developer, I need a Zustand dice store for ephemeral dice roll history and configuration, so that dice rolls are tracked in-memory during the session with no server persistence required.
...
- **Dice store purpose**: Manages dice roll history and provides a high-level `roll()` action that delegates to the dice engine (Epic 7). The store maintains a history of rolls for review and enables rolling from anywhere in the app. This is purely client-side ephemeral state — no API calls, no server persistence
- **Store state shape**:
...
- **Ephemeral state**: The roll history is kept in Zustand memory only — no persist middleware, no API calls, no database writes. History is lost on page refresh. This is intentional

## Tasks
- [ ] **T6.4.1** — Create `stores/diceStore.ts` with Zustand (no persist middleware). State: `{ rolls: DiceRoll[]; maxHistory: number }`
- [ ] **T6.4.2** — Implement `roll(dice: { type: DieType; count: number }[], modifier: number, label?: string, advantage?: boolean, disadvantage?: boolean): DiceRoll` — delegates to dice engine, adds result to history
- [ ] **T6.4.3** — Implement `clearHistory()` and `removeRoll(id)`
- [ ] **T6.4.4** — Write tests: Vitest for store logic — roll produces valid results within die range, advantage takes higher of 2d20, disadvantage takes lower, history accumulates and truncates at maxHistory
...
- The store is purely client-side Zustand — no API calls, no server persistence, no persist middleware
...
## Notes
- The dice store is purely client-side ephemeral Zustand state. It does not interact with the Django REST API or any persistence layer. Roll history exists only in memory for the current session
