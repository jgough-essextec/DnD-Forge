...
As a developer, I need automated tests that verify critical user flows work correctly. This story covers setting up the Playwright test suite with fixtures, then implementing 10 end-to-end tests covering: character creation (all 12 classes), character sheet rendering, dice rolling, session play, level-up, PDF export, campaign flow, import/export, and offline operation.
...
- **10 Critical E2E Flows**:
  ...
  9. Offline (install PWA, go offline, verify basic operations)
  10. (Implicit in fixtures) Data persistence verification
...
- Story 43.1 (PWA service worker for offline test)

## Notes

- Use Playwright's `page.context().route()` or `page.context().setOffline(true)` for offline testing
...
- IndexedDB state should be cleared between tests to ensure isolation
