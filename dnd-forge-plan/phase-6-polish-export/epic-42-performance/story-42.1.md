...
- **Expected Heavy Dependencies**: React (~45KB), jsPDF (~200KB), html2canvas (~100KB), framer-motion (~60KB), Three.js (~150KB if added), Dexie (~25KB), SRD data files
- **Code Split Targets**: Core (React, router, Zustand, Tailwind, shared), Gallery, Sheet, Wizard, DM, Dice, PDF
...
- App is split into at minimum 7 route-based chunks (Core, Gallery, Sheet, Wizard, DM, Dice, PDF)
...
- The PDF chunk (jsPDF + html2canvas) is ~300KB — it's critical this is only loaded on export action, not at initial load
