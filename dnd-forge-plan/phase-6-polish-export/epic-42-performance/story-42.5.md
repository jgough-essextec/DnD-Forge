...
As a developer, I need to verify the app handles edge cases at scale. This story covers testing with 100+ characters, large campaigns, long combat encounters, memory leak detection, and database/API limits with extreme data sizes.
...
- **Stress Test Scenarios**:
  - 100+ characters in gallery: test initial render, scroll performance, search filter, API response time
  ...
  - API/database limits: very long backstories (10,000+ chars), large inventories (100+ items), extensive spell lists
...
- [ ] **T42.5.1** — **100+ characters test:** create a script that generates 100+ characters with varied classes, races, levels, and equipment. Load the gallery and measure: initial render time, scroll performance, search filter responsiveness, API response time
...
- [ ] **T42.5.5** — **Database/API limits:** test with characters that have very long backstories (10,000+ chars), large inventories (100+ items), and extensive spell lists. Verify no API errors or database issues
...
- No API or database errors under stress conditions
...
- `should not encounter API or database errors under stress conditions`
...
- **Edge Cases**: PostgreSQL storage limits and API payload size limits not specifically tested or accounted for
...
- PostgreSQL storage is effectively unlimited for this use case; API payload size limits should be configured appropriately
