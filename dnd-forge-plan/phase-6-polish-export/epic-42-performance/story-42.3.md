...
- **API Response Optimization**: Batch reads via list endpoints instead of per-card API calls; leverage React Query caching
...
- [ ] **T42.3.4** — **API response optimization:** batch reads for the gallery (load all characters in one API call via GET /api/characters/, not one call per card). Leverage React Query caching for subsequent accesses. For the campaign dashboard, load all party characters in a single API call
...
- Gallery loads all characters in a single API call (not per-card)
- Campaign dashboard loads all party characters in a single API call
...
- `should load all gallery characters in a single API call (not per-card)`
...
- Avatar images served from the API should use proper caching headers and lazy loading to avoid blocking the main thread
